...
class SimpleIconManager:
    ...
    def _setup_colors(self):
        ...
        theme_lower = self.theme_name.lower() if self.theme_name else "light"
        if theme_lower in ["dark", "deep blue", "deep_blue", "jellyfish", "charcoal", "midnight", "night", "deep green", "deep_green"]:
    ...
    def set_theme(self, theme_name):
        ...
        self.theme_name = theme_name.lower() if theme_name else "light"
        self._setup_colors()
    ...
    def _create_svg_icon(self, icon_name, color, size=24):
        ...
        svg_file_map = {
            'laser': os.path.join(project_root, 'icons', 'laser_pointer-01.svg'),
            'board': os.path.join(project_root, 'icons', 'bord_svg.svg'),
            'select': os.path.join(project_root, 'icons', 'selector.svg'),
            'more': os.path.join(project_root, 'icons', '3_dot_options.svg'),
            'ellipse': os.path.join(project_root, 'icons', 'elipse.svg')
        }
        ...
        if icon_name in svg_file_map:
            ...
            if os.path.exists(svg_file_path):
                try:
                    ...
                    # Replace all color formats with the desired color
                    import re
                    
                    # First replace direct attribute values
                    ...
                    svg_content = svg_content.replace('stroke="black"', f'stroke="{color.name()}"')
                    ...
                    # Use regex for more complex patterns in style attributes
                    svg_content = re.sub(r'fill:\s*#000000', f'fill:{color.name()}', svg_content)
                    svg_content = re.sub(r'fill:\s*black', f'fill:{color.name()}', svg_content)
                    svg_content = re.sub(r'stroke:\s*#000000', f'stroke:{color.name()}', svg_content)
                    svg_content = re.sub(r'stroke:\s*black', f'stroke:{color.name()}', svg_content)
    ...
    def get_icon(self, icon_name, color='primary', size=24):
        ...
        theme_lower = self.theme_name.lower() if self.theme_name else "light"
        dark_themes = ["dark", "deep blue", "deep_blue", "jellyfish", "charcoal", "midnight", "night", "deep green", "deep_green"]
        
        if theme_lower in dark_themes and icon_name in ['laser', 'more']:
